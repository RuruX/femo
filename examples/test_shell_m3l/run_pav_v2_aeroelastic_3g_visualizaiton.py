...
## all real left wing geomety for meshing:
write_geometry = False

save_file = '/pav_wing/pav_wing_v2_3g_left_wing.iges'
left_wing_spatial_rep = SpatialRepresentation()
for name in structural_left_wing_names:
    left_wing_spatial_rep.primitives[name] = spatial_rep.primitives[name]

left_wing_spatial_rep.assemble()
if do_plots:
    left_wing_spatial_rep.plot(plot_types=['wireframe'])

if write_geometry:
    left_wing_spatial_rep.write_iges(cfile + save_file)

## make additional geometry for meshing
write_geometry = False
save_file = '/pav_wing/pav_wing_v2_3g_structue.iges'
...
for name in structural_left_wing_names:
    ...
    # order = 3
    # shape = 5
    # # knots = np.concatenate((np.zeros((order-1,)), np.linspace(0,1,shape-order+2), np.ones((order-1,))))
    # knots = bsf.generate_open_uniform_knot_vector(shape, order)
    # space_u = lg.BSplineSpace(name=name,
    #                         order=(order, order),
    #                         control_points_shape=(shape, shape, 3),
    #                         knots=(knots, knots))
    wing_spaces[name] = space
    wing_t_spaces[name] = space_t
    # wing_u_spaces[name] = space_u
    ...
    # displacement_coefficients[name] = m3l.Variable(name = name + '_u_coefficients', shape = (primitive.control_points.shape[0], primitive.control_points.shape[1], 3))
...
# wing_u_space_m3l = m3l.IndexedFunctionSpace(name='wing_space', spaces=wing_u_spaces)
...
# wing_displacement = m3l.IndexedFunction('wing_displacement', space = wing_u_space_m3l, coefficients=displacement_coefficients)
...
wing_displacement = index_functions(slwn_fixed, 'wing_displacement', space_u, 3)




name = structural_left_wing_names[1]
primitive = spatial_rep.get_primitives([name])[name].geometry_primitive
# print(primitive.order_u)
# print(primitive.shape)
# print(primitive.knots_u)
# exit()



## transfer mesh for wing displacements/stress/whatever
...
for name in structural_left_wing_names:
    for u in np.linspace(0,1,grid_num):
        for v in np.linspace(0,1,grid_num):
            transfer_para_mesh.append((name, np.array([u,v])))


# generate mapped array that corresponds to the above
# transfer_physical_nodes = wing_geo.evaluate(transfer_para_mesh) # this is for the future
...
for node in transfer_para_mesh:
    ...
    point_map_on_receiving_target = receiving_target.geometry_primitive.compute_evaluation_map(u_vec=np.array([node[1][0]]), v_vec=np.array([node[1][1]]))
...
# get rid of underscores and commas
for i in range(len(transfer_para_mesh)):
    transfer_para_mesh[i] = (transfer_para_mesh[i][0].replace(' ','_').replace(',',''), transfer_para_mesh[i][1])
...
if run_reprojection:
    file_name = '/pav_wing/pav_wing_v2_3g_mesh_data.pickle'
...
# exit()
with open(cfile + '/pav_wing/pav_wing_v2_3g_mesh_data.pickle', 'rb') as f:
...
coefficients_eval = {}
for key, value in displacement_coefficients.items():
    coefficients_eval[key] = value.value
...
# # Chamber surface
# left_wing_camber_surface = am.linspace(wing_upper_surface_wireframe, wing_lower_surface_wireframe, 1)
# left_wing_vlm_mesh_name = 'wing_vlm_mesh'
# sys_rep.add_output(left_wing_vlm_mesh_name, left_wing_camber_surface)

# # OML mesh
# wing_oml_mesh = am.vstack((wing_upper_surface_wireframe, wing_lower_surface_wireframe))
# left_wing_oml_mesh_name = 'wing_oml_mesh'
# sys_rep.add_output(left_wing_oml_mesh_name, wing_oml_mesh)
...
# test_disp = wing_displacement.evaluate(nodes_parametric)
...
# cruise_model.register_output(test_disp)
...
#################### end of m3l ########################
#### self-weight
# caddee_csdl_model.add_constraint(system_model_name+'Wing_rm_shell_model.rm_shell.aggregated_stress_model.wing_shell_aggregated_stress',upper=7E6,scaler=1E-6)
...
caddee_csdl_model.add_objective(system_model_name+'Wing_rm_shell_model.rm_shell.mass_model.mass', scaler=1e-2)
...
index = 0
# for name in structural_left_wing_names:
#     if index < 15 and (index%4 == 0 or index%4 == 3):
#         print(name)
#         print(index)
#     else:
#         name = name.replace(' ', '_').replace(',','')
#         coefficients[name] = sim['system_model.aircraft_trim.cruise_1.cruise_1.wing_thickness_evaluation.' + name + '_t_coefficients']
#     index += 1
...
for name in structural_left_wing_names:
    ...
    geo_coefficients[name] = geo_coeff

displacements = wing_displacement.compute(transfer_para_mesh, coefficients)
# locations = wing_geo.compute(transfer_para_mesh, geo_coefficients)
...
norm = colors.Normalize(np.min(v), np.max(v))
...
# optimizer = SLSQP(prob, maxiter=20, ftol=1E-5)
# optimizer.solve()
